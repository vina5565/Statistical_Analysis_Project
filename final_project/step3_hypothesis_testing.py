...
def hypothesis_1_1_cohort(df_students, df_volatility):
    ...
    if p_value < 0.05:
        ...
        print(f"  ✓ 결론: COVID 코호트의 변동성이 유의미하게 {direction}")
    else:
        print(f"  ✗ 결론: 두 코호트 간 유의미한 차이 없음")
    ...
    if f_p < 0.05:
        print(f"  ✓ 두 코호트의 분산이 유의미하게 다름")
    else:
        print(f"  ✗ 두 코호트의 분산 차이 없음")
    ...
    try:
        ...
        if model.pvalues['is_covid'] < 0.05:
            ...
            print(f"  ✓ COVID 코호트에서 변동성이 유의미하게 {direction}")
        else:
            print(f"  ✗ COVID 효과 유의미하지 않음")
    ...
    if p_value < 0.05 and diff_mean > 0:
        print(f"\n✅ 가설 채택: COVID 코호트의 변동성이 통계적으로 유의미하게 증가")
    elif p_value < 0.05 and diff_mean < 0:
        print(f"\n⚠️  가설 기각: COVID 코호트의 변동성이 오히려 감소")
    else:
        print(f"\n❌ 가설 기각: 두 코호트 간 유의미한 차이 없음")
...
def hypothesis_1_2_mediation(df_students, df_seteuk, df_volatility):
    ...
    가설: 내신 불안정성 증가는 세특의 텍스트 변화를 매개로 발생
          - 탐구·실험 키워드 감소
          - 온라인·원격 키워드 증가
    
    분석 모델: Mediation Analysis
    - X (독립변수): cohort (Pre-COVID=0, COVID=1)
    - M (매개변수): 탐구 키워드 감소, 온라인 키워드 증가
    - Y (종속변수): volatility
    """
    ...
    if model_a1.pvalues['is_covid'] < 0.05:
        ...
        print(f"  ✓ COVID 코호트에서 탐구 키워드 {direction}")
    
    # 경로 a: X → M (cohort → 온라인 키워드)
    print(f"\n경로 a': Cohort → 온라인 키워드")
    ...
    if model_a2.pvalues['is_covid'] < 0.05:
        ...
        print(f"  ✓ COVID 코호트에서 온라인 키워드 {direction}")
    ...
    print(f"\n탐구 키워드 매개 효과:")
    print(f"  a × b = {model_a1.params['is_covid']:.4f} × {model_b.params['kw_freq_exploration']:.4f}")
    print(f"  = {mediation_exploration:.4f}")
    
    # 온라인 키워드 매개 효과
    mediation_online = model_a2.params['is_covid'] * model_b.params['kw_freq_online']
    print(f"\n온라인 키워드 매개 효과:")
    print(f"  a × b = {model_a2.params['is_covid']:.4f} × {model_b.params['kw_freq_online']:.4f}")
    ...
    print(f"  매개 효과 (a×b): {total_mediation:.4f}")
    ...
    if mediation_significant and abs(total_mediation) > 0.01:
        if abs(direct_effect) < 0.01:
            print(f"\n✅ 완전 매개 (Full Mediation): 키워드 변화가 효과를 완전히 설명")
        else:
            print(f"\n✅ 부분 매개 (Partial Mediation): 키워드 변화가 효과를 부분적으로 설명")
    else:
        print(f"\n❌ 매개 효과 없음")
...
def main():
    ...
    try:
        ...
        print(f"✓ 세특: {len(df_seteuk)}건")
        print(f"✓ 변동성: {len(df_volatility)}건")
    except Exception as e:
        print(f"❌ 데이터 로드 실패: {e}")
    ...
    if results:
        ...
        print(f"\n✅ 결과 저장: data/results/hypothesis_results.csv")
